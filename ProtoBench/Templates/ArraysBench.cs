...
namespace ProtoBench
{
	...
	public partial class ArraysBench
	{
		...
		public void SerializeFloatArray64KStream()
		{
			...
			using (MemoryStream ms = new MemoryStream ()) {
				ProtoBuf.Serializer.Serialize<FloatArray64K> (ms, arr);
		...
		public void DeserializeFloatArray64KStream()
		{
			...
			using (MemoryStream ms = new MemoryStream (data)) {
				for (int i = 0; i < 250; i++) {
					ms.Position = 0;
					FloatArray64K des=ProtoBuf.Serializer.Deserialize<FloatArray64K>(ms);
				}
			}
			...
			using (MemoryStream ms = new MemoryStream (data)) {
				des1=ProtoBuf.Serializer.Deserialize<FloatArray64K>(ms);
			}
		...
		public void SerializeDoubleArray64KStream()
		{
			...
			using (MemoryStream ms = new MemoryStream ()) {
				ProtoBuf.Serializer.Serialize<DoubleArray64K> (ms, arr);
		...
		public void DeserializeDoubleArray64KStream()
		{
			...
			using (MemoryStream ms = new MemoryStream (data)) {
				for (int i = 0; i < 250; i++) {
					ms.Position = 0;
					DoubleArray64K des=ProtoBuf.Serializer.Deserialize<DoubleArray64K>(ms);
				}
			}
			...
			using (MemoryStream ms = new MemoryStream (data)) {
				des1=ProtoBuf.Serializer.Deserialize<DoubleArray64K>(ms);
			}
